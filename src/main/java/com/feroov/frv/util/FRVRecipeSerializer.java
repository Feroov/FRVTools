...
public class FRVRecipeSerializer implements RecipeSerializer<VCTableRecipe>
{
    private static List<Pair<Ingredient, Integer>> getIngredients(String pattern, Map<String, Pair<Ingredient, Integer>> keys, int width)
    {
        ...
        for (int i = 0; i < 4; i++) { pairList.add(Pair.of(Ingredient.EMPTY, 0)); }
    ...
    @Override
    public VCTableRecipe fromJson(ResourceLocation ResourceLocation, JsonObject jsonObject)
    {
        ...
        else if (pairList.size() > 4) { throw new JsonParseException("Too many ingredients for gun table recipe");
        }
        else
        {
            ItemStack itemStack = ShapedRecipe.itemFromJson(GsonHelper.getAsJsonObject(jsonObject, "result"))
                    .getDefaultInstance();
            return new VCTableRecipe(ResourceLocation, pairList.toArray(new Pair[0]), itemStack);
        }
    }

    @Override
    public void toNetwork(FriendlyByteBuf packetbuffer, VCTableRecipe gunTableRecipe)
    {
        for (int i = 0; i < 4; i++)
        {
    ...
    @Override
    public VCTableRecipe fromNetwork(ResourceLocation resourceLocation, FriendlyByteBuf packetbuffer)
    {
        @SuppressWarnings("unchecked")
        Pair<Ingredient, Integer>[] pairs = new Pair[4];
        for (int j = 0; j < 4; ++j)
        {
        ...
        return new VCTableRecipe(resourceLocation, pairs, output);
    }
}
