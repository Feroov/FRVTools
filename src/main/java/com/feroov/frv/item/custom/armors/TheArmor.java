...
public class TheArmor extends ArmorItem
{


    private static final Map<ArmorMaterial, MobEffect> MATERIAL_TO_EFFECT_MAP = new ImmutableMap.Builder<ArmorMaterial,
            MobEffect>().put(ModArmorMaterial.THE, MobEffects.DOLPHINS_GRACE).build();

    public TheArmor(ArmorMaterial p_40386_, EquipmentSlot p_40387_, Properties p_40388_)
    {
    ...
    private void evaluateArmorEffects(Player player)
    {
        for (Map.Entry<ArmorMaterial, MobEffect> entry : MATERIAL_TO_EFFECT_MAP.entrySet())
        {
            ...
            if(hasCorrectArmorOn(mapArmorMaterial, player))
            {
                ...
                player.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED, 550, 2));
            }
        }
    }
